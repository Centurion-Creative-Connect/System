...
namespace CenturionCC.System.Gimmick.SteelChallenge
{
    ...
    public class SteelChallengeGame : UdonSharpBehaviour
    {
        ...
        public void EndGame()
        {
            ...
            if (_hasFalseStart)
                GameEndDateTime = GameEndDateTime.AddSeconds(3);
            if (_hasFootFaults)
                GameEndDateTime = GameEndDateTime.AddSeconds(3);

            var span = GameEndDateTime.Subtract(GameStartDateTime);
            leaderboard.AddRecord(span, _lastWeaponName, Networking.LocalPlayer);
        ...
        public void OnTargetHit(SteelTarget target, DamageData damageData)
        {
            var shotPlayerId = damageData.DamagerPlayerId;
            ...
            _lastWeaponName = damageData.DamageType;
            target.hasHit = true;
            if (target == targets[0])
                EndGame();
        }
